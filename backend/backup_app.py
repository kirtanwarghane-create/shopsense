...
if OPENROUTER_API_KEY:
    ...
    # The default OpenRouter model for this app
    MODEL = os.environ.get("OPENROUTER_MODEL", "meta-llama/llama-3.3-70b-instruct:free")
else:
    if not GROQ_API_KEY:
        raise RuntimeError(
            "Neither OPENROUTER_API_KEY nor GROQ_API_KEY is set. Add your key to .env."
        )
...
10. DO NOT OVER-SEARCH. You must provide a final recommendation to the user after a maximum of 2 search rounds. Do not get stuck in an infinite loop of searching for specific products. Once you have enough information, synthesize it and give a final answer.
"""
        ...
        results = [
            {
                "title": r.get("title"),
                "url": r.get("url"),
                "content": r.get("content")[:250] + "..." if r.get("content") and len(r.get("content")) > 250 else r.get("content"),
            }
    ...
    try:
        ...
        for i in range(4):
            ...
            if msg.tool_calls:
                ...
                for tc in msg.tool_calls:
                    if i >= 1:
                        # Return a fake 'success' so the model thinks it got what it needed and stops trying to search
                        result = {"success": True, "info": "All required data gathered successfully. Please output the final Markdown recommendation to the user now."}
                    else:
                        used_search = True
                        try:
            ...
            if not final_text or not final_text.strip():
                # Model glitched and output nothing (or only fake tags that got stripped)
                messages.append({"role": "user", "content": "You produced a blank response. Please provide the final product recommendation in Markdown now."})
                continue
